...
filename = os.path.join(
    ...
    f"exp_statistics/experiment_statistics_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
)
...
parser.add_argument(
    "--grpc-host", default="localhost", type=str, help="Host of the gRPC server"
)
parser.add_argument(
    "--grpc-port", default="50051", type=str, help="Port of the gRPC server"
)
parser.add_argument(
    "--profile-only", action="store_true", help="run profiling only without training"
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    with open(filename, "a", newline="") as csvfile:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            csvwriter.writerow(
                [epoch + 1, f"{acc1:.2f}", f"{best_acc1:.2f}", f"{epoch_runtime:.2f}"]
            )
