        ...
        for i, sample_batch in enumerate(samples):
            ...
            for sample in sample_batch.samples:
                if isinstance(sample.image, bytes):
                    ...
                    (
                        transformed_data,
                        times_per_transformation,
                        transformed_sizes_per_transformation,
                    ) = self.preprocess_sample(
                        sample.image, sample.transformations_applied
                    )
                elif isinstance(sample.image, torch.Tensor):
                    image_tensor = sample.image
                    original_size = (
                        image_tensor.nelement() * image_tensor.element_size()
                    )
                    (
                        transformed_data,
                        times_per_transformation,
                        ...
                        image_tensor, sample.transformations_applied
                    )
    ...
    def preprocess_sample(self, sample, transformations_applied):
        ...
        for i in range(transformations_applied, len(transformations)):
            ...
            if isinstance(processed_sample, torch.Tensor):
                # For PyTorch tensors
                data_size = (
                    processed_sample.nelement() * processed_sample.element_size()
                )
        ...
        # LOGGER.debug("Transformation Times:", times)
...
if __name__ == "__main__":
    load_logging_config()
    profiler = Profiler(
        batch_size=200, dataset_path="imagenet", grpc_host="localhost", grpc_port=50051
    )
    gpu_throughput, io_throughput, cpu_preprocessing_throughput, sample_metrics = (
